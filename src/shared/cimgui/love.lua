...
function L.Init(format)
    Alpha8_shader = love.graphics.newShader([[
        ...
        vec4 effect(vec4 color, Image tex, vec2 texture_coords, vec2 screen_coords) {
            float alpha = Texel(Texture, texture_coords).r;
            return vec4(color.rgb, color.a*alpha);
        }
    ]], { debugname = "imgui alpha-8 shader" })
    ...
    ]], { debugname = "imgui default shader" })
...
local cmdBuffers = {}
local tempStr = ffi.new("char[?]", 64) -- 64 for a memory address
...
function L.RenderDrawLists()
    ...
    for i = 0, data.CmdListsCount - 1 do
        ...
        if vertexcount > cmdBuffer.max_vertexcount then
            ...
            ffi.copy(cmdBuffer.meshdata:getFFIPointer(), cmd_list.VtxBuffer.Data, data_size)
        ...
        ffi.copy(cmdBuffer.idx_buffer:getFFIPointer(), cmd_list.IdxBuffer.Data, indices_data_size)
